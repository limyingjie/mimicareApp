...
public class SerialSocket extends BluetoothGattCallback {
    ...
    public void disconnect() {
        ...
        device = null;
        canceled = true;
        Toast.makeText(context,"Disconnected from the BLE device",Toast.LENGTH_SHORT).show();
    ...
    public void connect(Context context, SerialListener listener, BluetoothDevice device) throws IOException {
        ...
        if (Build.VERSION.SDK_INT < 23) {
            ...
            Toast.makeText(context,"Connect to " + this.device,Toast.LENGTH_SHORT).show();
        } else {
        ...
        if (gatt == null){
            Toast.makeText(context,"Error in connecting " + this.device,Toast.LENGTH_SHORT).show();
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        for (BluetoothGattService gattService : gatt.getServices()) {
            if (gattService.getUuid().equals(FOOT_BLE_SERVICE)) {
                Log.d(TAG, "foot ble");
                //for(BluetoothGattCharacteristic characteristic : gattService.getCharacteristics())
                //    Log.d(TAG, "characteristic "+characteristic.getUuid());
                readCharacteristic = gattService.getCharacteristic(FOOT_BLE_R_NOTIFY);
                writeCharacteristic = gattService.getCharacteristic(FOOT_BLE_RW);
            }
        }
